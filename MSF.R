require(caret) 
require(e1071) 
tweetdat <- read.csv('MSF_Complete_No_Missing_Updated_10p.csv', as.is = T);
attach(eddat)
...
dm <- cbind(tweetdat["class"], scale(tweetdat[,c(2,4,5,6,7,8,9,11,12,13,15,16,18,19,20,21)]), tweetdat[c(1,3,10,14,17)])
...
for (i in seq(100))
{
  ...
  dm.navg <- avNNet(dm[c(2:22)], dm$class, subset = inTrain, size = 2, rang= 0.5, decay = 5e-6, maxit = 200, repeats = 50)
